...
class SenseHat(object):
    ...
    LED_MATRIX_I2C_ADDR = 0x46 # I2C address of the LED matrix
    ...
    def set_pixels(self, pixel_list):
        ...
        # Flatten the list in the format [R R R R ... G G G G ... B B B B ...]
        for i in range(0, 64, 8):
            r_values = [pixel[0] for pixel in pixel_list[i:i+8]]
            g_values = [pixel[1] for pixel in pixel_list[i:i+8]]
            b_values = [pixel[2] for pixel in pixel_list[i:i+8]]
        
            # Combine all the values into a single list
            byte_list = r_values + g_values + b_values
            
            byte_list = bytes(byte_list)

            self._bus.write_i2c_block_data(self.LED_MATRIX_I2C_ADDR, i*3, byte_list)
    ...
    def set_pixel(self, x, y, *args):
        ...
        self._bus.write_i2c_block_data(self.LED_MATRIX_I2C_ADDR, r_idx, bytes([pixel[0]]))
        self._bus.write_i2c_block_data(self.LED_MATRIX_I2C_ADDR, g_idx, bytes([pixel[1]]))
        self._bus.write_i2c_block_data(self.LED_MATRIX_I2C_ADDR, b_idx, bytes([pixel[2]]))
