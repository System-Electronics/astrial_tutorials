...
_TXB_EXIDE_M_16 = 0x08
_TXB_TXREQ_M = 0x08  # TX request/completion bit
...
_STAT_RXIF_MASK = 0x03
_RTR_MASK = 0x40

_STAT_TXIF_MASK = 0xA8
_STAT_TX0_PENDING = 0x04
_STAT_TX1_PENDING = 0x10
_STAT_TX2_PENDING = 0x40

_STAT_TX_PENDING_MASK = _STAT_TX0_PENDING | _STAT_TX1_PENDING | _STAT_TX2_PENDING
...
############ Misc Consts #########
_SEND_TIMEOUT_MS = 5  # 500ms
_MAX_CAN_MSG_LEN = 8  # Maximum message length is 8 bytes
...
class MCP2515:  # pylint:disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        *,
        ...
        self._debug = debug
        self._bus_device_obj = spi_device.SPIDevice()
    ...
    def _write_message(self, tx_buffer, message_obj):
        ...
        with self._bus_device_obj as spi:
            DLC = [len(message_obj.data)]
            to_send = [load_command] + list(self._id_buffer) + DLC + list(message_obj.data)
    ...
    def _write_id_to_register(self, register, can_id, extended=False):
        ...
        with self._bus_device_obj as spi:
            ...
            to_send = [_WRITE, register] + self._id_buffer[0:4]
            # spi.write(self._buffer, end=1)
            spi.write_readinto(
                to_send,  # because the reference does similar
                to_send
            )

            # send id bytes
            # spi.write(to_send, end=4)
    ...
    def _reset(self):
        ...
        # Try 3 times because it always fails on the first run
        while attempts < 3:
            try:
                with self._bus_device_obj as spi:
                    spi.write([_RESET])
    ...
    def _mod_register(self, register_addr, mask, new_value):
        ...
        to_send = [_BITMOD, register_addr, mask, new_value]
        # print("Senfing mod register: ", to_send)
        with self._bus_device_obj as spi:
            spi.write(to_send)
    ...
    def _read_register(self, regsiter_addr):
        ...
        with self._bus_device_obj as spi:
            spi.write_readinto(
                self._buffer, self._buffer, out_start=0, out_end=3, in_start=0, in_end=3
            )
